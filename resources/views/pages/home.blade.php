        ...
        <div class="mx-auto w-full max-w-7xl px-5 sm:px-8">
            ...
            <ul class="mt-14 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
                @foreach ($certificates as $certificate)
                    <li>
                        <button type="button" data-lightbox-trigger data-lightbox-gallery="certificates" data-lightbox-src="{{ asset('storage/'.ltrim($certificate->image, '/')) }}" data-lightbox-alt="{{ $certificate->alt }}" aria-label="{{ __('site.certificates.open', ['number' => $loop->iteration]) }}" class="group shadow-soft hover:shadow-lift focus-visible:outline-plum-600 block w-full overflow-hidden rounded-3xl bg-white transition-all duration-500 hover:-translate-y-1">
                            <span class="relative block">
                                <img src="{{ asset('storage/'.ltrim($certificate->image, '/')) }}" alt="" class="aspect-[3/4] w-full object-cover transition-transform duration-700 group-hover:scale-105">
